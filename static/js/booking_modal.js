...
class BookingModal {
  ...
  cacheElements() {
    this.elements = {
      ...
      serviceRequirements: this.modal.querySelector('#modalServiceRequirements'),
      requirementsList: this.modal.querySelector('#modalRequirementsList'),
      servicePreparation: this.modal.querySelector('#modalServicePreparation'),
      preparationNotes: this.modal.querySelector('#modalPreparationNotes'),
      serviceCancellation: this.modal.querySelector('#modalServiceCancellation'),
      cancellationPolicy: this.modal.querySelector('#modalCancellationPolicy'),
  ...
  populateServiceInfo() {
    ...
    // Update Requirements
    if (this.elements.serviceRequirements) {
      if (this.serviceData.requirements && this.serviceData.requirements.length > 0) {
        this.elements.serviceRequirements.style.display = 'block';
        this.elements.requirementsList.innerHTML = this.serviceData.requirements
          .map(req => `<li style="margin-bottom: 0.25rem;">${req}</li>`)
          .join('');
      } else {
        this.elements.serviceRequirements.style.display = 'none';
      }
    }

    // Update Preparation Notes
    if (this.elements.servicePreparation) {
      if (this.serviceData.preparation_notes) {
        this.elements.servicePreparation.style.display = 'block';
        this.elements.preparationNotes.textContent = this.serviceData.preparation_notes;
      } else {
        this.elements.servicePreparation.style.display = 'none';
      }
    }

    // Update Cancellation Policy
    if (this.elements.serviceCancellation) {
      if (this.serviceData.cancellation_policy) {
        this.elements.serviceCancellation.style.display = 'block';
        this.elements.cancellationPolicy.textContent = this.serviceData.cancellation_policy;
      } else {
        this.elements.serviceCancellation.style.display = 'none';
      }
    }
  ...
  async submitForm() {
    ...
    try {
      ...
      const response = await fetch(`/app/booking/request/${this.serviceData.id}/`, {
        method: 'POST',
        headers: {
      ...
      // Check if redirected (which means success in the new view)
      if (response.redirected) {
        window.location.href = response.url;
        return;
      }

      // If generic JSON response (fallback if view changed to JSON)
      if (response.headers.get('content-type')?.includes('application/json')) {
        const result = await response.json();
        if (response.ok && result.success) {
          this.showSuccessMessage(result.booking_code || 'APPT-REQUEST');
        } else {
          throw new Error(result.message || 'Failed to submit request');
        }
      } else {
        // Assume success if no error and redirect handled or HTML returned (shouldn't happen with correct view)
        // But for safety, reload or show success
        this.showSuccessMessage('PENDING-REVIEW');
      }
